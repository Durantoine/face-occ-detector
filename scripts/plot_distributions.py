"""Plot P_test / P_train densities and the regularized importance ratios (global/F/M).

Ported from the v36 pipeline; imports the continuous weighting from
src.utils.continuous_weight (KDE P_train + spline P_test, ratio P_test/(P_train+lam)).

    python scripts/plot_distributions.py --csv data/raw/train.csv --target joint
"""
...
from src.utils.continuous_weight import (
    CENTERS,
    DEFAULT_LAMBDA,
    N_BINS,
    P_TEST_PMF,
...
def main() -> None:
    ...
    p_train = _pr_grid_global(y.tobytes(), 0.018)
    ...
    c = ax[2]
    p_test_bad = np.clip(spline(grid), 1e-12, None); p_test_bad /= _trapz(p_test_bad, grid)
    p_train_n = p_train / _trapz(p_train, grid)
