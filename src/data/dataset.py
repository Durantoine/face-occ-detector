...
def _kfold_strata(y: np.ndarray, gender: np.ndarray) -> np.ndarray:
    # Same strata as p_train_val_indices: 20 fine bins for 0-40% (2% each) + 1 super-stratum 40-100%,
    # crossed with gender -> the val fold has the same gender x occlusion profile as the ptrain_strat split.
    g = (np.asarray(gender) >= 0.5).astype(int)
    yf = np.asarray(y, float)
    b = np.where(yf < 0.4, (yf * 50).astype(int), 20)
    return g * 21 + b


def kfold_val_indices(y: np.ndarray, gender: np.ndarray, n_folds: int, fold_idx: int, seed: int) -> np.ndarray:
    # Stratified K-fold. The full partition is built once from `seed` (independent of fold_idx), so
    # every fold job sees the same K disjoint folds; we just return fold #fold_idx as the val set.
    from sklearn.model_selection import StratifiedKFold
    strata = _kfold_strata(y, gender)
    skf = StratifiedKFold(n_splits=n_folds, shuffle=True, random_state=seed)
    for i, (_, val_idx) in enumerate(skf.split(np.zeros(len(y)), strata)):
        if i == fold_idx:
...
def make_splits(cfg: Config) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if getattr(cfg, "n_folds", 0) > 0 and getattr(cfg, "fold_idx", -1) >= 0:
        val_idx = kfold_val_indices(y, g, cfg.n_folds, cfg.fold_idx, seed=cfg.kfold_seed)
        mask = np.ones(len(df), dtype=bool); mask[val_idx] = False
        train_df = df.iloc[mask].reset_index(drop=True)
        val_df = df.iloc[val_idx].reset_index(drop=True)
        print(f"[data] K-fold {cfg.fold_idx + 1}/{cfg.n_folds} (kfold_seed={cfg.kfold_seed}): "
              f"val={len(val_df):,} held-out frames, train={len(train_df):,}", flush=True)
...
def make_loaders(cfg: Config, rank: int = 0, world_size: int = 1, pin_memory: bool = True):
    ...
    weight_fn = get_joint_weight_fn(y_pool, g_pool, target=cfg.is_target,
                                    alpha=cfg.eval_alpha, lam=cfg.is_lambda)
