...
def train(cfg: Config, optuna_trial=None, experiment: str = "faceocc",
    ...
    # v40: training is NATURAL (train_alpha=0, no IS reweight in loss/sampler). The ONLY reweighting left
    # is the SELECTION/objective metric, FIXED at (sel_is_target, eval_alpha) -> IDENTICAL for every trial
    # so Optuna ranks models on ONE metric (you can't search a hyperparameter that redefines the
    # objective). sel_is_target='joint' = per-gender IS, the faithful estimator of the per-gender final
    # metric (hc's global Y-ratio mis-weights each gender). eval_alpha=0.5 targets the intermediate proxy.
    _SEL_TGT = getattr(cfg, "sel_is_target", "joint")
    weight_fn_score = get_weight_fn(train_df[cfg.label_col].values,
                                    train_df[cfg.gender_col].values,
                                    target=_SEL_TGT, alpha=cfg.eval_alpha, lam=cfg.is_lambda)
    ...
    weight_fn_full = get_weight_fn(train_df[cfg.label_col].values, train_df[cfg.gender_col].values,
                                   target=_SEL_TGT, alpha=1.0, lam=cfg.is_lambda)
    # v40: log the IS score across the FULL alpha grid AND BOTH IS schemes (hc, joint) every epoch ->
    # interim->final + hc-vs-joint visibility, all independent of the searched is_target/train_alpha. The
    # OBJECTIVE is the single fixed point (sel_is_target, eval_alpha); the rest is monitoring only.
    _ALPHA_GRID = sorted({0.0, 0.25, 0.5, 0.75, 1.0, float(cfg.eval_alpha)})
    weight_fns_alpha = {(t, a): get_weight_fn(train_df[cfg.label_col].values, train_df[cfg.gender_col].values,
                                              target=t, alpha=a, lam=cfg.is_lambda)
                        for t in ("hc", "joint") for a in _ALPHA_GRID}
    ...
    with run_ctx:
        ...
        for epoch in range(cfg.epochs):
            ...
            if is_main:
                ...
                # v40: per-alpha IS score across the grid -> MLflow (per epoch) + carried in `m` so the
                # best-epoch values land in res -> Optuna user_attrs (logged in optimize.py).
                _alpha_metrics = {}
                for (_t, _a), _wf in weight_fns_alpha.items():
                    _sa = stratified_is_score(vp, vgt, vg, weight_fn=_wf)
                    _alpha_metrics[f"eval_score_{_t}_alpha_{_a}"] = float(_sa["challenge_score"])
                    _alpha_metrics[f"eval_gap_{_t}_alpha_{_a}"] = float(_sa["err_gap"])
                ml_metrics(None, None, _alpha_metrics, step=epoch)
                m.update(_alpha_metrics)
                print(f"  [Alpha-grid {_SEL_TGT}*] " + " | ".join(
                    f"a{_a}:{_alpha_metrics[f'eval_score_{_SEL_TGT}_alpha_{_a}']:.5f}" for _a in _ALPHA_GRID))
        ...
        if is_main:
            ...
            if _do_testpred:
                try:
                    ...
                    # Challenge submission format: filename, FaceOcclusion, gender="x" (gender not predicted).
                    # best.pt = the best-epoch checkpoint (loaded above), so these are best-epoch predictions.
                    pd.DataFrame({cfg.image_col: _tdf[cfg.image_col].values, cfg.label_col: _tp,
                                  cfg.gender_col: "x"}).to_csv(_csv, index=False)
...
if __name__ == "__main__":
    ...
    train(cfg, experiment=f"faceocc-{cfg.name}")
