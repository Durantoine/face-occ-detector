    ...
    if rank == 0:
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            try:
                ...
                # v40: best-epoch IS score/gap across the alpha grid x {hc, joint} (interim->final +
                # hc-vs-joint visibility); lets you re-rank on any scheme post-hoc.
                for k, v in res.items():
                    if k.startswith("eval_score_") or k.startswith("eval_gap_"):
                        trial.set_user_attr("best_" + k[len("eval_"):], float(v))
