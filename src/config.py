...
class Config:
    ...
    # K-fold cross-validation. n_folds>0 + fold_idx>=0 -> val = fold #fold_idx, train = the other K-1
    # folds. The partition is stratified on (gender x occlusion bin), identical across every fold job
    # (kfold_seed is independent of cfg.seed), so the K val sets are disjoint and tile the dataset once.
    n_folds: int = 0
    fold_idx: int = -1
    kfold_seed: int = 20240613
