...
- **Reweighting** ([`src/utils/distribution.py`](src/utils/distribution.py)) — importance weight `w = P_test_target / (P_train + is_lambda)`, a **label-shift** correction on `(Y, gender)` only (never on the image `x`). **v40 decouples it from training**: training is **natural** (`train_alpha=0`, just the metric weight `1/30+y`, full effective sample size → no low-occ collapse). The IS reweighting survives **only in the selection/objective metric**, FIXED at (`sel_is_target=joint` per-gender, `eval_alpha=0.5` intermediate target) so every Optuna trial is ranked on the **same** metric. Both `hc`/`joint` schemes across the full alpha grid are logged each epoch (monitoring + post-hoc re-ranking). The final **marginal** P_test alignment is a post-hoc transport step ([`scripts/recover_ptest.py`](scripts/recover_ptest.py)), not baked into the loss.
...
python src/optimize.py sapiens-final               # Optuna HPO (reads configs/architectures/<name>.yaml)
python src/predict.py  sapiens-final               # submission.csv  (cols: filename, FaceOcclusion, gender="x")
python src/predict.py  <mlflow_run_id>               # predict from a specific trial's best.pt (best epoch)
```

Configs live in [`configs/architectures/*.yaml`](configs/architectures/) (one `name:`, `model:`, `data:`, `training:`, `optuna:` per file). Active configs (v40): `sapiens-final`, `dino-vitb16`, `efficientnet-mini-local`.
...
```bash
# ENSTA (SLURM, 1× H100) — one script per model
sbatch scripts/optimize_sapiens.slurm               # sapiens-final
sbatch scripts/optimize_dino.slurm                  # dino-vitb16 (batch 256)

# Lyceum (non-SLURM VM) — default config = sapiens-final
...
python -m src.ensemble_predict sapiens-final --topk 5 --out submission.csv
