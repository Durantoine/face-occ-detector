...
#
# Arguments:
#   arch_name: Name of the architecture config (e.g., sapiens-final)
#   --optimize: If present, runs src/optimize.py instead of src/train.py

set -euo pipefail

ARCH_NAME="dino-vitb16-hc"
MODE="train"
...
if [ -n "${BUCKET_URL}" ]; then
    ...
    # MLflow artifacts (Pretrains only - Experiment 1). Robust: clean stale .gstmp, retry the parallel
    # rsync (transient .gstmp races over large model.pth abort it), then serial-cp any iBOT encoder
    # referenced by the configs that is still missing/empty (dropped encoder -> "No artifacts found").
