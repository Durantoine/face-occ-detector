...
from src.utils.distribution import p_test_val_indices, p_train_val_indices
...
_AUG_LEVELS: dict = {
    "off": None,
    "light":  {"flip": 0.5, "color": (0.7, 0.10), "blur": (0.0, 0.0), "noise": (0.0, 0.00), "affine": (0.0, 0.0)},
    "medium": {"flip": 0.5, "color": (0.8, 0.20), "blur": (0.25, 1.2), "noise": (0.3, 0.02), "affine": (0.0, 0.0)},
}
...
_VAL_SAMPLERS = {"ptest": p_test_val_indices, "ptrain_strat": p_train_val_indices}
...
def append_augmented(train_df: pd.DataFrame, val_df: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    if "source_filename" in aug.columns:
        keep = ~aug["source_filename"].isin(set(val_df[cfg.image_col]))
        leak = int((~keep).sum()); aug = aug[keep].copy()
    aug_root = Path(getattr(cfg, "aug_image_dir", "data/aug")).resolve()
    aug[cfg.image_col] = aug[cfg.image_col].map(lambda f: str(aug_root / Path(str(f)).name))
...
def make_loaders(cfg: Config, rank: int = 0, world_size: int = 1, pin_memory: bool = True):
    ...
    aug_w = np.clip(total_weights_raw, AUG_S_MIN, 1.0)  # interesting (w>=1) -> full aug, surplus -> less
    ...
    dl = dict(num_workers=cfg.num_workers, pin_memory=pin_memory,
              persistent_workers=False)  # workers reaped each epoch -> no FD accumulation across HPO trials ([Errno 24])

    # v30: Use StratifiedNoReplacementSampler with raw weights
