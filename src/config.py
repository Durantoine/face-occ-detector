...
class Config:
    ...
    val_mode: str = "ptrain"
    full_data: bool = False  # 100%-data retrain: train on ALL frames, val = in-sample monitoring only, save the final epoch
    ...
    batch_remainder: str = "redistribute"  # v40 group_batch leftover: "redistribute" (round-robin -> full coverage) | "drop" (drop_last)
    ...
    # v40: alpha of the IS weight applied to the TRAINING loss + sampler, DECOUPLED from eval_alpha
    # (which now only drives selection). 0.0 = natural training (metric weight 1/30+y only, no
    # covariate-shift reweight, full effective sample size); >0 reintroduces the P_test tilt.
    train_alpha: float = 0.0
    # v40: FIXED IS scheme for the SELECTION/objective metric (NEVER searched) -> every trial is ranked
    # on the SAME metric. 'hc' = global IS: LOW-VARIANCE -> stable trial ranking + intra-trial checkpoint
    # selection (joint's per-gender estimate is high-variance in the scarce female high-occ tail). Both
    # hc AND joint are logged every epoch, so the FINAL trial pick stays re-rankable on joint post-hoc.
    sel_is_target: str = "hc"
    epochs: int = 10
    batch_size: int = 64
    lr: float = 3e-5
    ...
    seed: int = 42
    rotate_seed: bool = False  # v38: per-trial seed = seed + trial.number (HPO only) -> diverse top-K for ensembling
    bf16: bool = True
    num_workers: int = 8
