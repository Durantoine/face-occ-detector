...
def mask_glasses(rng: np.random.Generator, target: float) -> Image.Image:
    """Glasses / sunglasses: two lens ellipses + bridge + arms.

    Scaled so the union of lenses ≈ target area. When target > 0.20 the lenses
    grow large (oversized sunglasses) which — combined with a second mask via
    stacking — can reach Y ≥ 0.30.
    """
    H = W = WORK
    # Derive lens half-axes from target: 2 * π * lens_h * lens_w ≈ target * H * W
    # Fix aspect ratio lens_w = 2 * lens_h  →  lens_h = sqrt(target / (2π * 2/H))
    f_h = float(np.sqrt(target / (4 * np.pi))) * H
    lens_h = int(np.clip(f_h, 0.06 * H, 0.22 * H))
    lens_w = int(lens_h * rng.uniform(1.6, 2.2))

    eye_y  = int(H * rng.uniform(0.35, 0.45))
    gap    = int(W * rng.uniform(0.03, 0.07))   # nose bridge gap
    cx_l   = W // 2 - gap // 2 - lens_w
    cx_r   = W // 2 + gap // 2 + lens_w
    tilt   = rng.uniform(-0.18, 0.18)

    mask = np.zeros((H, W), dtype=bool)
    yy, xx = np.ogrid[:H, :W]
    for cx in [cx_l, cx_r]:
        dy = (yy - eye_y).astype(float)
        dx = (xx - cx).astype(float)
        ellipse = ((dy * np.cos(tilt) + dx * np.sin(tilt)) / lens_h) ** 2 + \
                  ((-dy * np.sin(tilt) + dx * np.cos(tilt)) / lens_w) ** 2 <= 1
        mask |= ellipse

    # Bridge
    bx0, bx1 = max(0, cx_l + lens_w), min(W, cx_r - lens_w)
    if bx1 > bx0:
        mask[eye_y - 2:eye_y + 2, bx0:bx1] = True

    # Arms (temple bars extending to image edges)
    arm_y = eye_y - lens_h // 4
    arm_t = max(2, lens_h // 8)
    if cx_l - lens_w > 0:
        mask[arm_y - arm_t:arm_y + arm_t, :cx_l - lens_w] = True
    if cx_r + lens_w < W:
        mask[arm_y - arm_t:arm_y + arm_t, cx_r + lens_w:] = True

    return _to_pil(mask, feather=3)
...
PROMPTS = {
    ...
    "glasses": [
        "large dark sunglasses covering eyes and nose, photorealistic, fashion eyewear, sharp focus",
        "oversized tinted sunglasses masking the upper face, realistic portrait, natural lighting",
        "big round sunglasses, photorealistic, detailed lenses, sharp focus",
    ],
}
...
def _print_distribution(df: pd.DataFrame) -> None:
    bins   = [0.0, 0.10, 0.20, 0.30, 0.35, 0.40, 0.45, 0.50, 1.01]
    labels = ["0.00-0.10", "0.10-0.20", "0.20-0.30",
              "0.30-0.35", "0.35-0.40", "0.40-0.45", "0.45-0.50", "0.50+"]
    df = df.copy()
    df["_bin"] = pd.cut(df["FaceOcclusion"], bins=bins, labels=labels, right=False)
    total = len(df)
    M = df[df["gender"] == 1]
    F = df[df["gender"] == 0]
    BAR = 30
    ...
    print(f"\n{'='*62}")
    print(f"  DISTRIBUTION FaceOcclusion générée  (n={total}, "
          f"M={len(M)} F={len(F)})")
    print(f"  {'Bin':<11}  {'Tot':>4}  {'M':>4}  {'F':>4}  Histogram")
    print(f"{'='*62}")
    for b in labels:
        sub  = df[df["_bin"] == b]
        subM = M[M["_bin"] == b]
        subF = F[F["_bin"] == b]
        n, nm, nf = len(sub), len(subM), len(subF)
        bar_m = "▓" * int(nm / total * BAR)
        bar_f = "░" * int(nf / total * BAR)
        marker = " ◄ cible" if b in ("0.30-0.35","0.35-0.40","0.40-0.45","0.45-0.50","0.50+") else ""
        print(f"  {b:<11}  {n:>4d}  {nm:>4d}  {nf:>4d}  {bar_m}{bar_f}{marker}")
    print(f"{'='*62}")
    hi = df[df["FaceOcclusion"] >= 0.30]
    print(f"  Y ≥ 0.30 : {len(hi)} ({len(hi)/total*100:.1f}%)  "
          f"M={len(hi[hi.gender==1])} F={len(hi[hi.gender==0])}\n")
...
def build_pipeline(model: str, device: str, dtype):
    ...
    if is_xl:
        ...
        pipe = StableDiffusionInpaintPipeline.from_pretrained(
            model, torch_dtype=dtype, variant="fp16", safety_checker=None)
    pipe = pipe.to(device)
...
def main() -> None:
    ...
    ap.add_argument("--male-ratio", type=float, default=0.667,
                    help="Fraction of generated samples that are male (gender=1). "
                         "Default 0.667 (2/3 M) to compensate severe male under-representation at Y>0.30.")
    ...
    ap.add_argument("--area-min", type=float, default=0.25,
                    help="Lower bound on the occluder area fraction. 0.25 with clean_label_max=0.10 "
                         "gives final labels ≥ 0.30 (start of P_test pénurie zone).")
    ap.add_argument("--area-max", type=float, default=0.50,
                    help="Upper bound on the occluder area fraction. 0.45 with clean_label_max=0.10 "
                         "keeps final labels ≤ 0.50 = support boundary of P_test (ratio=0 beyond).")
    ...
    ap.add_argument("--extra-image-dir", type=Path, default=None,
                    help="Directory of unlabeled face images (e.g. 112×112 ms1m-arcface). "
                         "Used as additional source pool. Label=0, gender random at --male-ratio.")
    ap.add_argument("--extra-ratio", type=float, default=0.75,
                    help="Fraction of n-samples drawn from --extra-image-dir (default 0.75).")
    ap.add_argument("--blur-prob", type=float, default=0.2,
                    help="Probability of Gaussian blur (σ 1.5-3.0) applied post-inpainting.")
    ap.add_argument("--flip-prob", type=float, default=0.3,
                    help="Probability of horizontal flip applied before inpainting.")
    ap.add_argument("--types", nargs="+",
                    default=["hair", "headwear", "scarf", "hand", "glasses"],
    ...
    # --- Pool A : train clean (stratified by gender) ---
    n_extra = int(args.n_samples * args.extra_ratio) if args.extra_image_dir else 0
    n_train = args.n_samples - n_extra
    n_train_m = int(n_train * args.male_ratio)
    n_train_f = n_train - n_train_m
    pool_m = clean[clean["gender"] == 1]
    pool_f = clean[clean["gender"] == 0]
    if len(pool_m) == 0 or len(pool_f) == 0:
        sys.exit("ERROR: no samples for one gender in train clean pool")
    train_sampled = pd.concat([
        pool_m.sample(n=n_train_m, replace=len(pool_m) < n_train_m, random_state=args.seed),
        pool_f.sample(n=n_train_f, replace=len(pool_f) < n_train_f, random_state=args.seed),
    ]).assign(is_extra=False)
    print(f"[pool A] {n_train} train images  M={n_train_m} F={n_train_f}")

    # --- Pool B : unlabeled extra images ---
    extra_records: list[dict] = []
    if args.extra_image_dir and n_extra > 0:
        extra_files = (sorted(args.extra_image_dir.rglob("*.jpg")) +
                       sorted(args.extra_image_dir.rglob("*.png")) +
                       sorted(args.extra_image_dir.rglob("*.webp")))
        if not extra_files:
            sys.exit(f"ERROR: no images found in {args.extra_image_dir}")
        chosen_idx = rng.integers(0, len(extra_files), size=n_extra)
        genders = (rng.random(n_extra) < args.male_ratio).astype(float)
        extra_records = [{"filename": str(extra_files[i]), "FaceOcclusion": 0.0,
                          "gender": g, "is_extra": True}
                         for i, g in zip(chosen_idx, genders)]
        n_em = int(genders.sum()); n_ef = n_extra - n_em
        print(f"[pool B] {n_extra} unlabeled images from {args.extra_image_dir}  M={n_em} F={n_ef}")

    # Merge and shuffle both pools
    all_records = train_sampled.to_dict("records") + extra_records
    rng.shuffle(all_records)
    sampled = pd.DataFrame(all_records).reset_index(drop=True)
    print(f"[sample] total={len(sampled)}  (A={n_train} B={n_extra})")
    ...
    def build_job(idx, init, orig, gender, source, otype, retry=0):
        ...
        area = float(mask_arr.mean())

        # Stack a second mask if single mask doesn't reach area_min (e.g. glasses alone)
        otype_label = otype
        if area < args.area_min:
            remaining = float(rng.uniform(args.area_min - area, args.area_max - area))
            remaining = max(0.08, min(remaining, 0.35))
            others = [t for t in args.types if t != otype] or args.types
            otype2 = random.choice(others)
            mask2_arr = np.asarray(MASK_FUNCS[otype2](rng, remaining)) > 127
            combined = mask_arr | mask2_arr
            mask = _to_pil(combined, feather=2)
            area = float(combined.mean())
            otype_label = f"{otype}+{otype2}"

        # Horizontal flip augmentation (label-preserving)
        if rng.random() < args.flip_prob:
            init = init.transpose(Image.FLIP_LEFT_RIGHT)
            mask = mask.transpose(Image.FLIP_LEFT_RIGHT)

        prompt_key = otype if otype in PROMPTS else otype.split("+")[0]
        return {
            "idx": idx, "init": init, "mask": mask, "type": otype_label, "retry": retry,
            "prompt": random.choice(PROMPTS[prompt_key]), "area": area, "orig": orig,
    ...
    def run_batch(b: list[dict]) -> list[dict]:
        ...
        for j, gen_img in zip(b, result):
            ...
            # blur augmentation (label-preserving — degrades sharpness, not occlusion)
            if rng.random() < args.blur_prob:
                sigma = float(rng.uniform(1.5, 3.0))
                comp = comp.filter(ImageFilter.GaussianBlur(sigma))
    ...
    while True:
        ...
        while len(batch) < args.batch_size and not exhausted:
            try:
                ...
                exhausted = True
                break
            is_extra = bool(getattr(row, "is_extra", False))
            src = Path(row.filename) if is_extra else Path(args.image_dir) / row.filename
            try:
        ...
        for j in run_batch(batch):
            if j["retry"] < args.max_retries:
                # retry with a fresh mask/seed; strip stacked label back to base type
                base_type = j["type"].split("+")[0]
                retry_q.append(build_job(j["idx"], j["init"], j["orig"], j["gender"],
                                         j["source"], base_type, retry=j["retry"] + 1))
                n_retried += 1
            else:
                n_dropped += 1
