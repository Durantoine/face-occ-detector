...
def evaluate(raw_model, loader, device, val_mode: str, weight_fn=None, mem_fmt=torch.contiguous_format):
    ...
    metrics = {
        ...
        "m_raw": m_raw,
        "m_strat": m_strat,
        "primary_is_strat": val_mode != "ptest" and weight_fn is not None
    }
...
def train(cfg: Config, optuna_trial=None, experiment: str = "faceocc",
    ...
    loss_fn = ChallengeLoss(lambda_gap=cfg.lambda_gap, adaptive=cfg.lambda_adaptive,
                            lam_lr=cfg.lambda_lr, lam_min=cfg.lambda_min,
                            lam_max=cfg.lambda_max, threshold=cfg.lambda_threshold)
    ...
    with run_ctx:
        ...
        for epoch in range(cfg.epochs):
            ...
            for i, b in enumerate(train_loader):
                ...
                with sync_ctx:
                    # ChallengeLoss handles the fairness gap internally; lambda adapts once per
                    # epoch on the validation gap (loss_fn.update_lambda below), not per batch.
                    if use_amp:
                        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            if is_main:
                ...
                new_lam = loss_fn.update_lambda(m["eval_err_diff"])
                
                print(f"\nepoch {epoch:2d} | VALIDATION (mode: {cfg.val_mode}) | lam: {new_lam:.2f}")
                ...
                if m["eval_challenge_score"] < best["eval_challenge_score"]:
                    best = {**m, "epoch": epoch}
                    torch.save(raw_model.state_dict(), out / "best.pt"); bad = 0
                    if cfg.save_qualitative_k > 0:
