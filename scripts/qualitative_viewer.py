...
with st.sidebar:
    ...
    _default_idx = next((i for i, lbl in enumerate(_exp_labels) if "v22" in lbl.lower()), 0)
...
if _tp_df is None or len(_tp_df) == 0:
    ...
    if len(_vals) == 0:
        ...
        fig_tp.add_trace(go.Scatter(x=_grid, y=_ptest, mode="lines", name="P_test cible (continue)",
        ...
        st.caption("Densité **continue** des prédictions test (best epoch) vs cible **P_test**. "
                   "Si le modèle est bien aligné sur le shift, la courbe bleue épouse la rouge.")
